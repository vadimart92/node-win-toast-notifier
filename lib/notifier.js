...
export class Notifier {
    ...
    async _subscribeForEvents() {
        const response = await fetch(await this._getUrl(`status-stream?from=${this._lastStatusMessageNumber}`), {
    ...
    async remove(notification) {
        ...
        let result = await fetch(await this._getUrl(`notification?id=${notification.id}`), {
            headers: this._getHeaders(),
            method: 'DELETE',
        });
        if (result.ok) {
