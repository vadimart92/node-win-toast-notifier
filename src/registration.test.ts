...
import {
    registerAppId,
    registerAppIdUsingShortcut,
    unRegisterAppId,
    unRegisterAppIdUsingShortcut,
} from './register-app-id.js';
        ...
        describe('when app not registered', () => {
            test('registerAppId', async () => {
                await registerAppId(
                    'notifier-test-1',
                    'Some test app',
                    `D:\\dev\\GitHub\\cimon\\src\\cimon-desktop\\icons\\green\\icon.png`
                );
            });
            ...
            describe('when user not allow registration', () => {
                test('registerAppId', async () => {
                    await expect(
                        registerAppId('notifier-test-1', 'Some test app')
                    ).rejects.toMatch(
                        /Exit code: 101(.|\s)*The operation was canceled by the user/gm
                    );
                });
            });
        });
